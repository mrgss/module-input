...
static mrb_value mouse_update(mrb_state *mrb, mrb_int capa) {
    SDL_PumpEvents();
...
mouse_position(mrb_state *mrb, mrb_int capa) {
    ...
    mrb_ary_push(mrb, res, mrb_fixnum_value(mouse_x));
    mrb_ary_push(mrb, res, mrb_fixnum_value(mouse_y));
    return res;
}

static mrb_value mrgss_mouse_x(mrb_state *mrb, mrb_int capa) {
    return mrb_fixnum_value(mouse_x);
}

static mrb_value mrgss_mouse_y(mrb_state *mrb, mrb_int capa) {
    return mrb_fixnum_value(mouse_y);
}

static mrb_value left_click (mrb_state* mrb, mrb_value self)
{
  mrb_int button;
 ...
 */
void mrgss_mouse_init(mrb_state *mrb) {
    struct RClass *hw = mrb_define_module_under(mrb, mrgss_module(), "Mouse");
    ...
    mrb_define_class_method(mrb, hw, "x", (mrb_func_t) mrgss_mouse_x, MRB_ARGS_NONE());
    mrb_define_class_method(mrb, hw, "y", (mrb_func_t) mrgss_mouse_y, MRB_ARGS_NONE());
